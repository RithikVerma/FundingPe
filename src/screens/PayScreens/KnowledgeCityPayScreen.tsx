...
import { fetchKnowledgeCityList, toggleFavorite, getFavorites } from '../../utils/api';
...
const KnowledgeCityPayScreen: React.FC = () => {
  ...
  const loadFavorites = useCallback(async () => {
    try {
      ...
      if (Object.keys(newFavorites).length === 0 && showNoFavoritesToast) {
        // write the toast message for no favorites
       // ToastAndroid.show('No favorites found. Add some Knowledge City items to your favorites', ToastAndroid.SHORT);
  ...
  // Fetch Knowledge City data from API
  useEffect(() => {
    const loadKnowledgeCityData = async () => {
      setLoading(true);
      setError(null);
      try {
        const searchQuery = searchText.trim() && searchBy === 'Name' ? searchText : undefined;
        const data = await fetchKnowledgeCityList(searchQuery);
        
        // Transform API data to match KnowledgeItem type
        const knowledgeCityData = data.map(item => ({
          id: item.id.toString(),
          name: item.name,
          address: item.address || item.location || 'N/A',
          image: item.image ? { uri: item.image } : require('../../assets/icons/knowledge1.jpg'),
        ...
        setKnowledgeItems(knowledgeCityData);
      } catch (err) {
        console.error('Failed to load Knowledge City data:', err);
        setError('Failed to load Knowledge City data. Please try again.');
      } finally {
        setLoading(false);
      }
    };
    
    loadKnowledgeCityData();
    loadFavorites();
  }, [searchText, searchBy, loadFavorites]);

  // Filter knowledge city items based on search text and searchBy
  const filteredKnowledgeItems = useMemo(() => {
    if (!searchText.trim()) return knowledgeItems;
  ...
  }, [searchText, searchBy, knowledgeItems]);

  // Handle Pay Now button click with logging
  const handlePayNow = (item: KnowledgeItem) => {
    console.log('Selected Knowledge City for payment:', item.name);
    console.log('Knowledge City details:', {
      id: item.id,
      name: item.name,
      Mobile: item.Mobile,
      Whatsapp: item.Whatsapp,
  ...
  const handleViewDescription = (item: KnowledgeItem) => {
  ...
  const handleToggleFavorite = async (knowledgeId: string) => {
    try {
      // Start loading for this specific item
      setToggleLoading(prev => ({...prev, [knowledgeId]: true}));
      
      // Call API to toggle favorite status
      const response = await toggleFavorite(knowledgeId);
      
      if (response && response.success) {
        setFavorites(prev => {
          const updated = {...prev};
          // Toggle favorite status in local state
          if (updated[knowledgeId]) {
            delete updated[knowledgeId]; // Remove if it was a favorite
          } else {
            updated[knowledgeId] = true; // Add if it wasn't a favorite
          }
          return updated;
        });
      ...
      setToggleLoading(prev => ({...prev, [knowledgeId]: false}));
    }
  };
  ...
  // Render knowledge item for list view
  const renderKnowledgeItem = ({ item }: { item: KnowledgeItem }) => {
    ...
    return (
      <View style={styles.knowledgeCard}>
        <View style={styles.knowledgeTopSection}>
          <View style={styles.knowledgeImageContainer}>
            {typeof item.image === 'object' && item.image !== null ? (
              <Image 
                source={item.image} 
                style={styles.knowledgeImage}
                defaultSource={require('../../assets/icons/knowledge1.jpg')} 
              />
            ) : (
              <Image 
                source={require('../../assets/icons/knowledge1.jpg')} 
                style={styles.knowledgeImage} 
              />
            )}
          </View>
          <View style={styles.knowledgeInfo}>
            <Text style={styles.knowledgeName}>{item.name}</Text>
            <Text style={styles.knowledgeLocation}>{item.address}</Text>
  ...
  if (showPayment && selectedKnowledge) {
    return (
      <PaymentDetailsScreen
        id={selectedKnowledge.id}
        title="Knowledge City Donation"
        name={selectedKnowledge.name}
        address={selectedKnowledge.address}
        paymentType="Knowledge City Donation"
        mobile={selectedKnowledge.Mobile || ''}
        upiId={selectedKnowledge.upi || ''}
        whatsapp={selectedKnowledge.Whatsapp || ''}
        qrImage={selectedKnowledge.QR ? selectedKnowledge.QR.toString() : null}
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Knowledge City Pay</Text>
        </View>
        ...
        {loading && (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading Knowledge City data...</Text>
          </View>
        )}
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyText}>No Knowledge City found</Text>
              </View>
            }
          />
        )}
...
export default KnowledgeCityPayScreen; 