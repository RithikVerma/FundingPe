...
import { fetchSamajikWorkList, toggleFavorite, getFavorites } from '../../utils/api';

type SamajikWorkItem = {
  id: string | number;
  name: string;
  address: string;
  image: any;
...
const SamajikWorkPayScreen: React.FC = () => {
  const navigation = useNavigation<NavigationProp>();
  const [searchText, setSearchText] = useState('');
  const [showPayment, setShowPayment] = useState(false);
  const [selectedSamajik, setSelectedSamajik] = useState<SamajikWorkItem | null>(null);
  ...
  const [samajikItems, setSamajikItems] = useState<SamajikWorkItem[]>([]);
  ...
  // Fetch Samajik Work data from API
  useEffect(() => {
    const loadSamajikWorkData = async () => {
      setLoading(true);
      setError(null);
      try {
        ...
        const data = await fetchSamajikWorkList(searchQuery);
        
        // Transform API data to match SamajikWorkItem type
        const samajikWorkData = data.map(item => ({
          id: item.id.toString(),
          name: item.name,
          address: item.address || item.location || 'N/A',
          image: item.image ? { uri: item.image } : require('../../assets/icons/samajik1.jpg'),
        ...
        setSamajikItems(samajikWorkData);
      } catch (err) {
        console.error('Failed to load Samajik Work data:', err);
        setError('Failed to load Samajik Work data. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Filter samajik work items based on search text and searchBy
  const filteredSamajikItems = useMemo(() => {
    if (!searchText.trim()) return samajikItems;
    
    const searchLower = searchText.toLowerCase();
    return samajikItems.filter(item => {
      if (searchBy === 'Name') {
  ...
  }, [searchText, searchBy, samajikItems]);

  // Handle Pay Now button click with logging
  const handlePayNow = (item: SamajikWorkItem) => {
    console.log('Selected Samajik Work for payment:', item.name);
    console.log('Samajik Work details:', {
      id: item.id,
      name: item.name,
      Mobile: item.Mobile,
      Whatsapp: item.Whatsapp,
    ...
    setSelectedSamajik(item);
    setShowPayment(true);
  };

  // Handle Comety Description button click
  const handleViewDescription = (item: SamajikWorkItem) => {
  ...
  const handleToggleFavorite = async (samajikId: string) => {
    try {
      // Start loading for this specific item
      setToggleLoading(prev => ({...prev, [samajikId]: true}));
      
      // Call API to toggle favorite status
      const response = await toggleFavorite(samajikId);
      
      if (response && response.success) {
        setFavorites(prev => {
          const updated = {...prev};
          // Toggle favorite status in local state
          if (updated[samajikId]) {
            delete updated[samajikId]; // Remove if it was a favorite
          } else {
            updated[samajikId] = true; // Add if it wasn't a favorite
          }
          return updated;
        });
      ...
      setToggleLoading(prev => ({...prev, [samajikId]: false}));
    }
  };
  ...
  // Render samajik work item for list view
  const renderSamajikItem = ({ item }: { item: SamajikWorkItem }) => {
    ...
    return (
      <View style={styles.samajikCard}>
        <View style={styles.samajikTopSection}>
          <View style={styles.samajikImageContainer}>
            {typeof item.image === 'object' && item.image !== null ? (
              <Image 
                source={item.image} 
                style={styles.samajikImage}
                defaultSource={require('../../assets/icons/samajik1.jpg')} 
              />
            ) : (
              <Image 
                source={require('../../assets/icons/samajik1.jpg')} 
                style={styles.samajikImage} 
              />
            )}
          </View>
          <View style={styles.samajikInfo}>
            <Text style={styles.samajikName}>{item.name}</Text>
            <Text style={styles.samajikLocation}>{item.address}</Text>
  ...
  // If payment screen is showing, render PaymentDetailsScreen with WhatsApp
  if (showPayment && selectedSamajik) {
    return (
      <PaymentDetailsScreen
        id={selectedSamajik.id}
        title="Social Work Donation"
        name={selectedSamajik.name}
        address={selectedSamajik.address}
        paymentType="Social Work Donation"
        mobile={selectedSamajik.Mobile || ''}
        upiId={selectedSamajik.upi || ''}
        whatsapp={selectedSamajik.Whatsapp || ''}
        qrImage={selectedSamajik.QR ? selectedSamajik.QR.toString() : null}
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Samajik Work Pay</Text>
      </View>
      ...
      {loading && (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading Samajik Work data...</Text>
        </View>
      )}
      ...
      {!loading && !error && (
        <FlatList
          data={filteredSamajikItems}
          keyExtractor={(item) => item.id.toString()}
          renderItem={renderSamajikItem}
          contentContainerStyle={styles.listContainer}
          showsVerticalScrollIndicator={false}
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Icon name="search-off" size={52} color="#8A2BE2" />
              <Text style={styles.emptyText}>No Samajik Work found</Text>
            </View>
          }
        />
      )}
...
export default SamajikWorkPayScreen; 